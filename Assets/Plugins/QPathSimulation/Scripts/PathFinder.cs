...
public class PathFinder : MonoBehaviour {
    ...
    // 
    public bool onlyRandomSpawnNearPlayer = false;
    ...
    void OnlySpawnRandomNearPlayer() {
        PathFollower follower = SpawnCar();
        List<Path> paths = RandomPathNearPosition(mainCamera.transform.position, searchRadius);
        follower.Follow(paths);
    }
    ...
    PathFollower SpawnCar() {
        ...
        GameObject go = Instantiate(_CarModels[UnityEngine.Random.Range(0,_CarModels.Count)]);
    ...
    List<Path> RandomPathNearPosition(Vector3 position, float searchRadius) {
        List<Node> nearbyNodes = graphData.centers
            .Where(node => Vector3.Distance(position, node.position) <= searchRadius)
            .ToList();

        if (nearbyNodes.Count < 2) {
            // If there are not enough nodes nearby, you might want to increase the search radius or handle this case.
            return null;
        }

        List<Node> nodes = null;
        while (nodes == null || nodes.Count == 0) {
            int a = Random.Range(0, nearbyNodes.Count);
            int b = a;
            while (b == a) {
                b = Random.Range(0, nearbyNodes.Count);
            }
            Node spawn = nearbyNodes[a];
            Node target = nearbyNodes[b];
            nodes = FindShortedPathSynchronousInternal(spawn.ID, target.ID);
        }
    ...
    IEnumerator Spawn() {
        ...
        while (true) {
            if (spawning && maxCars > amount) {
                if(onlyRandomSpawnNearPlayer)
                {
                    OnlySpawnRandomNearPlayer();
                    amount++;
                }
                else if (randomSpawn) {
                    SpawnRandom();
