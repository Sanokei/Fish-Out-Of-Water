...
public class PathFollower : MonoBehaviour {
    ...
    IEnumerator FollowRoutine(List<Path> path) {
        ...
        transform.SetPositionAndRotation(path[index].PosOfA, Quaternion.RotateTowards(transform.rotation, Quaternion.LookRotation(path[index].PosOfB - transform.position), 400f));
        ...
        while (index < end) {
            ...
            if (dist >= 0.1f) {
                transform.SetPositionAndRotation(transform.position + transform.forward * velocity * Time.deltaTime * 1, Quaternion.RotateTowards(transform.rotation, Quaternion.LookRotation(target - transform.position), 100f * Time.deltaTime));
            }
    ...
    void Update()
    {
        if(Vector3.Distance(PathFinder.Instance.mainCamera.transform.position, transform.position) > PathFinder.Instance.searchRadius)
        {
            PathFinder.Instance.amount--;
            Destroy(gameObject);
        }
    }
}
